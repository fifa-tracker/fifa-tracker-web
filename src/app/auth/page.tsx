...
export default function AuthPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const success = await signUp(formData.name, formData.email, formData.password, formData.username);
      if (success) {
        router.push('/');
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#0f1419] text-white flex flex-col">
      ...
      <footer className="text-center py-6 px-4">
        <p className="text-gray-400 text-sm">© 2024 FIFA Tracker. All rights reserved.</p>
      </footer>
    </div>
  );
} 